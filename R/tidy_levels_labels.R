...
#' @param argcheck An environment generated by \code{ArgumentCheck::newArgCheck}.
#'   Under normal circumstances, this is passed from \code{dust}.  If \code{NULL},
#'   as in the case it is run outside of \code{dust}, a new environment is
#'   created and the argument check is completed within \code{tidy_levels_labels}.
#'   
...
tidy_levels_labels <- function(object, 
  ...
  if (is.null(argcheck)) argcheck <- ArgumentCheck::newArgCheck()
  
  numeric_level <- ArgumentCheck::match_arg(numeric_level,
                                           c("term", "term_plain", "label"),
                                           argcheck = argcheck)
  
  descriptors <- ArgumentCheck::match_arg(descriptors,
                                          c("term", "term_plain", "label",
                                            "level", "level_detail"),
                                          several.ok = TRUE,
                                          argcheck = argcheck)
  
  if (independent_check) ArgumentCheck::finishArgCheck(argcheck)
